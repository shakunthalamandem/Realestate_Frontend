...
const ExpenseList = ({
  ...
  const styles =
    tone === "green"
      ? { text: "text-green-700", bg: "bg-green-100", heading: "text-green-700" }
      : { text: "text-red-700", bg: "bg-red-100", heading: "text-red-700" };

  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <p className={`mb-4 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] ${styles.heading}`}>
        {title}
      </p>
      <div className="space-y-3">
        {Array.from({ length: Math.max(rows?.length ?? 0, 3) }).map((_, index) => {
          ...
          return (
            ...
            >
              <div>
                <p className="min-h-[20px] text-sm font-medium text-slate-900">{item?.label ?? ""}</p>
                <p className="min-h-[16px] text-xs text-slate-500">{item?.reason ?? ""}</p>
              </div>
              <div className="text-right">
                <p className={`min-h-[20px] text-sm font-bold ${styles.text}`}>{item?.change ?? ""}</p>
                <p className="min-h-[16px] text-xs text-slate-500">{item?.amount ?? ""}</p>
              </div>
            </div>
          );
        })}
...
const ExpenseIntelligence = ({ data }: { data?: IcExpenseIntelligenceData | null }) => {
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div className="mb-4 flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 p-4">
            ...
            <div>
              ...
              <p className="min-h-[16px] text-xs text-slate-500">{efficiency?.statusSubtitle ?? ""}</p>
            </div>
          </div>

          <div className="space-y-3 text-sm">
            <div className="flex items-center justify-between">
              <span className="text-slate-500">Structural savings</span>
              <span className="font-semibold text-slate-900">
                {efficiency?.structuralSavings != null ? `${efficiency.structuralSavings}%` : ""}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between">
              <span className="text-slate-500">Timing-driven</span>
              <span className="font-semibold text-slate-900">
                {efficiency?.timingDriven != null ? `${efficiency.timingDriven}%` : ""}
              </span>
            </div>
            <div className="h-2 rounded-full bg-[#E5E7EB]">
              <div
            ...
            <p className="border-t border-gray-200 pt-2 text-xs text-slate-500">{efficiency?.note ?? ""}</p>
          </div>
        </div>
      </div>
    </section>
